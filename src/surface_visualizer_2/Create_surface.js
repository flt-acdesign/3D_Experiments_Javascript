function surface_smooth(fun, scene, shadowGenerator, recipy = {"color3" : [.4, .5, .6] , "alpha" : 1, "wireframe" : false}    ) {
  var size = 100;
  var data = {
      positions: [],
      indices: [],
...
function valueToRainbowColor(value, min, max) {
      ...
      for (var z = -size; z <= size; z++) {
          ...
          if (x < size && z < size) {
              ...
              data.indices.push(i + 1, i + 2 * size + 1, i + 2 * size + 2);
                      ...
                      color = valueToRainbowColor(y, -100, 100)
  ...
  material.diffuseColor = new BABYLON.Color3(recipy.color3[0], recipy.color3[1], recipy.color3[2] ); // Use the color provided in the function argument
  ...
  material.alpha = recipy.alpha
  material.wireframe = recipy.wireframe
